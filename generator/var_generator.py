...
from generator.var_challenge import Challenge
...
def map_var_list_to_dict(var_list, with_id):
    """
    ...
    for var in var_list:
        if with_id:
            challenge = Challenge(var.name, var.id)
            var_dict[challenge] = var.generated_value
        else:
            var_dict[var.name] = var.generated_value
    return var_dict


def generate(variable_list, seed, with_id = False):
    """
    ...
    return map_var_list_to_dict(list_of_generated_objects, with_id)
